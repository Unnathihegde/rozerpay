...
export default function App() {
  ...
  const [payment, setPayment] = useState<"idle" | "pending" | "approval" | "ready" | "failed" | "completed">("idle");
  ...
  const assistantMatches = useMemo(() => {
    ...
    const requestedCategories = categories.filter((item) => item !== "All" && normalizedText.includes(item.toLowerCase()));
    const budgetMatch = normalizedText.match(/(?:under|below|less than)\s*[₹rs. ]*([\d,]+)/i);
    const maxPrice = budgetMatch ? Number(budgetMatch[1].replace(/,/g, "")) * 100 : null;
    return products.filter((product) => {
      const categoryMatches = requestedCategories.length === 0 || requestedCategories.includes(product.category);
      const priceMatches = maxPrice === null || product.price <= maxPrice;
      return categoryMatches && priceMatches;
    });
  }, [assistantText, categories, products]);
  const assistantBestMatch = assistantMatches[0];
  ...
  const refreshOrderState = async (orderId = order?.order_id) => {
    ...
    try {
      ...
      if (status === "paid") setPayment("completed");
